-- Multi-instance frps: instance detail edit page
-- 每个 instance section 拥有一份完整的 frps 配置：bind / auth / vhost / webServer 等
...
local sid = arg[1]
if not sid or uci:get("frps", sid) ~= "instance" then
	luci.http.redirect(luci.dispatcher.build_url("admin/services/frps/instances"))
	return
end

m = Map("frps", "%s - %s" % { translate("Frps"), translate("实例配置") },
	translate("配置好绑定端口 / 鉴权 token 后，勾选「启用此实例」并保存即可启动该 frps 实例。每个实例的 bindPort / 管理面板端口 / 防火墙规则相互独立。"))
m:append(Template("frps/theme"))

s = m:section(NamedSection, sid, "instance")
s.addremove = false
s.anonymous = true

s:tab("general",   translate("常规"))
s:tab("basic",     translate("基础"))
s:tab("advanced",  translate("高级"))
s:tab("dashboard", translate("管理面板"))
...
o = s:taboption("general", ListValue, "set_firewall", translate("防火墙通信规则"),
	translate("检测：启动服务，无规则将建立，停止服务时不删除<br/>强制：启动时删除并重建，停止时删除"))
o:value("no", translate("不操作"))
o:value("check", translate("检测"))
o:value("force", translate("强制"))
o.default = "no"

o = s:taboption("general", Value, "tcp_ports", translate("防火墙通信规则-TCP端口"),
	translate("多端口号用空格隔开，下同"))
...
-- === basic ===
o = s:taboption("basic", Value, "bindAddr", translate("绑定地址"),
	translate("留空，即所有地址（含IPv6）"))
...
o = s:taboption("basic", ListValue, "auth__method", translate("鉴权方式"),
	translate("留空默认 token，若用 oidc 请使用「高级」中的「额外选项」添加参数"))
o:value("", translate("（空）"))
o:value("token")
o:value("oidc")
...
o = s:taboption("basic", Value, "kcpBindPort", translate("KCP绑定端口"),
	translate("UDP端口用于kcp协议，建议不要与QUIC端口冲突；留空以禁用kcp"))
o.datatype = "port"

o = s:taboption("basic", Value, "quicBindPort", translate("QUIC绑定端口"),
	translate("UDP端口用于quic协议，建议不要与KCP端口冲突；留空以禁用quic"))
o.datatype = "port"

o = s:taboption("basic", Value, "vhostHTTPPort", translate("虚拟主机HTTP端口"),
	translate("如果希望支持虚拟主机，则必须设定 http 或 https 端口"))
o.datatype = "port"

o = s:taboption("basic", Value, "vhostHTTPSPort", translate("虚拟主机HTTPS端口"),
	translate("注意：frpc 默认禁用了TLS第一个自定义字节，可能影响端口号复用，建议查阅官方文档"))
o.datatype = "port"
...
o = s:taboption("advanced", Value, "subDomainHost", translate("子域名后缀"),
	translate("如果subDomainHost不为空，例如frps.com；可在frpc中对类型为http(s)的代理设置subdomain，若设为test，路由将使用test.frps.com"))
o.datatype = "host"
...
o = s:taboption("advanced", DynamicList, "extra_setting", translate("额外选项"),
	translate("点击添加列表，一行一条，写入frp的配置文件的通用部分末尾；格式错误可能导致无法启动服务"))
o.placeholder = "option = value"

-- === dashboard ===
o = s:taboption("dashboard", Value, "webServer__addr", translate("面板地址"),
	translate("默认本机访问；要远程访问按需设置"))
o.datatype = "host"
...
o = s:taboption("log", Flag, "std_redirect", translate("重定向标准输出"),
	translate("Frp 的标准输出、标准错误重定向到日志文件"))
o:depends("enable_logging", "1")

o = s:taboption("log", Value, "log__to", translate("日志文件"),
	translate("填写文件路径，留空相当于填入 console（日志打印在标准输出中）"))
o:depends("enable_logging", "1")
o.default = "/var/log/frps.log"

o = s:taboption("log", ListValue, "log__level", translate("日志等级"),
	translate("留空默认：info"))
o:depends("enable_logging", "1")
o:value("", translate("（空）"))
...
o = s:taboption("log", Value, "log__maxDays", translate("日志保存天数"),
	translate("留空默认 3 天（不含当天），会按日期命名文件，1天1个"))
o:depends("enable_logging", "1")
o.datatype = "uinteger"
o.placeholder = "3"

o = s:taboption("log", Flag, "log__disablePrintColor", translate("禁用日志颜色"),
