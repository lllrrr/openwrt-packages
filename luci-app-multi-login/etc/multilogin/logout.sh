#!/bin/sh
# Package-managed logout compatibility wrapper.
...
usage_error() {
	printf '%s\n' 'multilogin logout: invalid arguments; --password is not supported' >&2
	exit 4
}
mwan3=''
account=''
...
if [ -z "$mwan3" ] || [ -z "$account" ]; then usage_error; fi
validate_core || {
	printf '%s\n' 'multilogin logout: portal core is unavailable' >&2
	exit 5
}
set -- logout --mwan3 "$mwan3" --account "$account"
