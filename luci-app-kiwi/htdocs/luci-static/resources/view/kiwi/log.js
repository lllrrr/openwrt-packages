...
function injectCSS() {
	...
	'}',
	'.kiwi-log .log-btn:hover{background:var(--bs-secondary-bg,rgba(127,127,127,.12))}',
...
/* procd 将进程输出转发到 syslog 时格式为 "kiwi[PID]:"，需兼容可选的 [PID] 部分 */
var SYSLOG_RE = /(?:kernel\.|user\.|daemon\.)?(?:notice|info|warn(?:ing)?|err(?:or)?|debug|emerg|alert|crit)\s+kiwi(?:\[\d+\])?\s*:\s*(.*)/i;
var SYSLOG_LEVEL_RE = /(notice|info|warn(?:ing)?|err(?:or)?|debug|emerg|alert|crit)\s+kiwi(?:\[\d+\])?\s*:/i;
