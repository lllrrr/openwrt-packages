...
* **轻量级存储**：使用 SQLite 存储历史数据，支持自动保留期限和按需清理。
...
* **v1.1.16**: 修复 HTTP 失败误记为低延迟、TCPing 端口解析、空数据响应、命令拼接和前端 XSS 问题；增加历史数据分桶、自动清理、配置校验和刷新防重叠。
* **v1.1.14**: 算法升级。平滑模式采用 **5点移动中位数滤波 (Moving Median)** + **EMA** 双重算法，在去除孤立噪点（毛刺）的同时保持曲线平滑，完美还原真实网络趋势。
...
* **数据保留天数**：默认保留 90 天，后台每小时自动清理过期记录。
...
* **检测间隔**：默认为 60 秒，允许范围为 5～86400 秒；建议保持 60 秒或更长。
...
* 长时间范围会在服务端自动聚合，每个节点最多返回约 5000 个数据点。
...
项目代码采用 MIT License；内置的 Apache ECharts 采用 Apache-2.0 License，详见 `THIRD_PARTY_NOTICES.md`。
