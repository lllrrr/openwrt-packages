...
check_core_latest_version() {
	...
	core_latest_ver="$(wget -T10 -qO- 'https://api.github.com/repos/你的用户名/luci-app-unblockneteasemusic-gdstudio/commits?sha=main&path=core' | jsonfilter -e '@[0].sha')"
...
update_core() {
	...
	for file in $(wget -T10 -qO- "https://api.github.com/repos/你的用户名/luci-app-unblockneteasemusic-gdstudio/contents/core" | jsonfilter -e '@[*].path')
	do
		wget -T10 "https://fastly.jsdelivr.net/gh/你的用户名/luci-app-unblockneteasemusic-gdstudio@$core_latest_ver/$file" -qO "$UNM_DIR/core/${file##*/}"
		[ -s "$UNM_DIR/core/${file##*/}" ] || {
	...
	do
		wget -T10 "https://fastly.jsdelivr.net/gh/你的用户名/luci-app-unblockneteasemusic-gdstudio@$core_latest_ver/$cert" -qO "$UNM_DIR/core/$cert"
		[ -s "$UNM_DIR/core/${cert}" ] || {
