...
/* ── injectCSS ─────────────────────────────────── */
function injectCSS() {
	if (document.getElementById('panda-log-css')) return;
	...
	el.textContent = [
		...
		'}',
		'.panda-log .log-bar .spacer{flex:1}',
		'.panda-log .log-bar input[type=search]{',
		...
		'  font-size:.75rem;',
		'}',
		'.panda-log .log-bar input[type=search]:focus{',
		...
		'}',
		'.panda-log .log-btn:hover{background:rgba(127,127,127,.15)}',
		'.panda-log .log-btn.active{',
		'  background:rgba(64,160,43,.18);border-color:rgba(64,160,43,.35);',
		'}',
		'.panda-log .log-btn.danger:hover{',
		'  background:rgba(243,139,168,.25);border-color:rgba(243,139,168,.4);',
		'}',
		'.panda-log .log-btn svg{width:14px;height:14px;flex-shrink:0}',
		'.panda-log .log-muted{opacity:.55;font-style:italic;font-size:.75rem}',
		'.panda-log .log-stat{font-size:.75rem;opacity:.7}',
		'.panda-log .log-stat strong{opacity:1;font-weight:700}',
	].join('');
...
return view.extend({
	...
	render() {
		...
		/* ── Log content ────────────────────────── */
		var logPre = E('pre', { id: 'panda-log-content' });
		var logPane = E('div', { 'class': 'log-pane' }, [logPre]);

		/* ── Status display ─────────────────────── */
		var statusEl = E('span', { 'class': 'log-stat', id: 'panda-log-status' }, '--');
			...
			click: function() {
				return clearLogRpc().then(function(result) {
					...
					if (result) { doReset(); }
						...
						return fs.exec_direct('/bin/sh', ['-c', ': > /var/log/ddns.log'])
							.then(doReset)
							.catch(function(err) {
		...
		/* ── Root ───────────────────────────────── */
		var root = E('div', { 'class': 'cbi-map panda-log' }, [
			E('h2', {}, '日志'),
		...
		self.__pollHandle = poll.add(function() {
			return fs.read_direct('/var/log/ddns.log', 'text')
	...
	__renderLog: function() {
		var el = document.getElementById('panda-log-content');
		if (!el) return;
		...
		/* Update status */
		var st = document.getElementById('panda-log-status');
		if (st) {
