...
test("rpcd script: hub_share, hub_my_shares, hub_update, hub_delete handlers exist", () => {
  assert.ok(rpcd.includes('"hub_share")'));
  assert.ok(rpcd.includes('"hub_my_shares")'));
  assert.ok(rpcd.includes('"hub_update")'));
  assert.ok(rpcd.includes('"hub_delete")'));
});

test("rpcd script: hub_share, hub_my_shares, hub_update, hub_delete registered in list branch", () => {
  assert.ok(rpcd.includes('json_add_object "hub_share"'));
  assert.ok(rpcd.includes('json_add_object "hub_my_shares"'));
  assert.ok(rpcd.includes('json_add_object "hub_update"'));
  assert.ok(rpcd.includes('json_add_object "hub_delete"'));
});

test("rpcd script: hub_share validates name/author/description with has_control_char before use", () => {
  const shareBranchMatch = rpcd.match(/"hub_share"\)([\s\S]*?)\n\t;;/);
  ...
  assert.match(body, /has_control_char\s+"\$author"/);
  assert.match(body, /has_control_char\s+"\$description"/);
});

test("rpcd script: hub_share ensures device identity, builds payload, POSTs, and records the id", () => {
  const shareBranchMatch = rpcd.match(/"hub_share"\)([\s\S]*?)\n\t;;/);
  ...
  assert.match(body, /hub_shares/);
});

test("rpcd script: hub_my_shares reads hub_shares and fetches each id", () => {
  const branchMatch = rpcd.match(/"hub_my_shares"\)([\s\S]*?)\n\t;;/);
  assert.ok(branchMatch);
  const body = branchMatch[1];
  assert.match(body, /DEVICE_DIR\/hub_shares/);
  assert.match(body, /hub_http_get\s+"\/api\/v1\/themes\/aurora\/configs\/\$sid"/);
});

// --- FINAL-REVIEW: hub_my_shares must never rewrite hub_shares (it is the
// only local record of what this device has published, and this handler
// runs on every gallery.js page load). hub_http_get returns nonzero for both
// a network failure and a genuine 404, indistinguishably -- treating a
// transient failure as "gone" and dropping the id here would be
// unrecoverable. Id removal is hub_delete's job alone, on a confirmed
// successful delete. ---

// Bounded explicitly by the next case branch's pattern (rather than the
// generic "\n\t;;" terminator other tests in this file use), since that
// terminator is single-tab-indented but every per-branch ";;" in this
// script is actually double-tab-indented ("\t\t;;") -- the lazy "\n\t;;"
// match otherwise skips straight past it and keeps matching into
// subsequent branches (hub_update, hub_delete, ...), which would make a
// negative assertion like "never rewrites hub_shares" false-fail on
// hub_delete's legitimate rewrite.
test("rpcd script: hub_my_shares never rewrites $DEVICE_DIR/hub_shares (no mv/write onto it)", () => {
  const branchMatch = rpcd.match(/"hub_my_shares"\)([\s\S]*?)\n\t"hub_update"\)/);
  assert.ok(branchMatch);
  const body = branchMatch[1];
  assert.ok(
    !/>\s*"?\$DEVICE_DIR\/hub_shares"?/.test(body) &&
      !/mv\s+\S+\s+"?\$DEVICE_DIR\/hub_shares"?/.test(body),
    "hub_my_shares must not write/overwrite $DEVICE_DIR/hub_shares -- only hub_delete may",
  );
});

test("rpcd script: hub_my_shares splices the flat hub_http_get body straight into items (no 'data' select)", () => {
  const branchMatch = rpcd.match(/"hub_my_shares"\)([\s\S]*?)\n\t"hub_update"\)/);
  assert.ok(branchMatch);
  const body = branchMatch[1];
  assert.ok(!/json_select\s+data/.test(body), "hub_http_get's own response has no 'data' wrapper to select");
  assert.match(body, /my_shares_item_list="\$my_shares_body"/);
  assert.match(body, /my_shares_item_list="\$my_shares_item_list,\$my_shares_body"/);
});
...
test("rpcd script: hub_delete guards id charset, DELETEs with device_token, and removes the id from hub_shares", () => {
...
test("acl: hub_share, hub_update, hub_delete granted under write.ubus; hub_my_shares under read.ubus", () => {
  const acljson = JSON.parse(acl);
  const writeMethods = acljson["luci-app-aurora"].write.ubus["luci.aurora"];
  const readMethods = acljson["luci-app-aurora"].read.ubus["luci.aurora"];
  ...
  assert.ok(readMethods.includes("hub_my_shares"));
});
