...
The generated schema-5 configuration assigns every emitted matcher a stable `ruleTag` (dual-list sections use `<section>:domain` and `<section>:ip`), expresses the virtual Default row through `routing.finalOutboundTag`, routes each DNS server through its own `outboundTag`, and maps selected server tags to IPv4/IPv6 NFTSets. Each WireGuard node's single remote peer runs inside BypassCore and participates in the same transactional snapshot lifecycle. Status, route explanation, DNS resolution, NFTSet health, Observatory data, and readiness are read from the running core over a mode-0600 Unix socket, avoiding duplicate GeoData and DNS initialization for diagnostics.
