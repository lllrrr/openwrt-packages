module("luci.sxray.util_xray", package.seeall)
...
function gen_config_server(node)
	...
	if node.outbound_node then
		...
		if node.outbound_node == "_iface" and node.outbound_node_iface then
			...
			local outbound_node_t = uci:get_all("sxray", node.outbound_node)
			...
			outbound = require("luci.sxray.util_xray").gen_outbound(nil, outbound_node_t, "outbound")
		end
		if outbound then
	...
	local config = {
		log = {
			-- error = "/tmp/etc/sxray_server/log/" .. user[".name"] .. ".log",
...
function gen_config(var)
	...
	if (remote_dns_udp_server and remote_dns_udp_port) or (remote_dns_tcp_server and remote_dns_tcp_port) then
		...
		if direct_dns_udp_server or direct_dns_tcp_server then
			local domain = {}
			local nodes_domain_text = sys.exec('uci show sxray | grep ".address=" | cut -d "\'" -f 2 | grep "[a-zA-Z]$" | sort -u')
