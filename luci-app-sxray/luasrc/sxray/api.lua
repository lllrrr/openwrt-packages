module("luci.sxray.api", package.seeall)
local com = require "luci.sxray.com"
nixio = require "nixio"
fs = require "nixio.fs"
...
function set_cache_var(key, val)
	sys.call(string.format('. /usr/share/sxray/utils.sh ; set_cache_var %s "%s"', key, val))
end

function get_cache_var(key)
	local val = sys.exec(string.format('. /usr/share/sxray/utils.sh ; echo -n $(get_cache_var %s)', key))
	if val == "" then val = nil end
	return val
end

function get_new_port()
	local cmd_format = ". /usr/share/sxray/utils.sh ; echo -n $(get_new_port %s tcp,udp)"
...
function curl_auto(url, file, args)
	...
	if localhost_proxy == "1" then
		return curl_base(url, file, args) -- 当路由器本机开启代理时，采用sxray规则进行访问
	else
		local return_code, result = curl_proxy(url, file, args)
		if not return_code or return_code ~= 0 then
...
function get_bin_version_cache(file, cmd)
	sys.call("mkdir -p /tmp/etc/sxray_tmp")
	
	if not file or file == "" then
		log("Error: Empty file path provided for version check")
		return ""
	end
	...
	if not md5 or md5 == "" then
		log("Error: Failed to calculate MD5 for file: " .. file)
		return ""
	end
	
	local cache_file = "/tmp/etc/sxray_tmp/" .. md5
	
	if fs.access(cache_file) then
		local cached_version = sys.exec("echo -n $(cat " .. cache_file .. ")")
		if cached_version and cached_version ~= "" then
			log("Using cached version for " .. file .. ": " .. cached_version)
			return cached_version
		end
	end
	
	sys.call("echo 1 > /proc/sys/vm/overcommit_memory 2>/dev/null")
	
	log("Getting version for: " .. file .. " with command: " .. cmd)
	
	local version = sys.exec(string.format("echo -n $(%s %s 2>&1)", file, cmd))
	
	if not version or version == "" then
		log("Error: Failed to get version for " .. file .. " - empty result")
		return ""
	end
	
	if version:find("fatal error") or version:find("error") or version:find("failed") then
		log("Error: Version command failed for " .. file .. ": " .. version)
		return ""
	end
	
	sys.call("echo '" .. version .. "' > " .. cache_file)
	log("Successfully got version for " .. file .. ": " .. version)
...
function get_app_path(app_name)
	if not app_name or app_name == "" then
		log("Error: Empty app_name provided to get_app_path")
		return nil
	end
	
	if not com[app_name] then
		log("Error: Unknown app_name: " .. app_name)
		return nil
	end
	
	local def_path = com[app_name].default_path
	local config_key = app_name:gsub("%-","_") .. "_file"
	local path = uci_get_type("global_app", config_key)
	
	if path and #path > 0 then
		log("Using custom path for " .. app_name .. ": " .. path)
		return path
	else
		log("Using default path for " .. app_name .. ": " .. def_path)
		return def_path
	end
end
...
function get_app_version(app_name, file)
	if not app_name or app_name == "" then
		log("Error: Empty app_name provided to get_app_version")
		return ""
	end
	
	if not com[app_name] then
		log("Error: Unknown app_name in get_app_version: " .. app_name)
		return ""
	end
	
	if file == nil then
		file = get_app_path(app_name)
	end
	
	if not file or file == "" then
		log("Error: No file path available for " .. app_name)
		return ""
	end
	
	if not com[app_name].cmd_version then
		log("Error: No version command defined for " .. app_name)
		return ""
	end
	
	log("Getting version for " .. app_name .. " from file: " .. file)
...
function to_move(app_name,file)
	...
	local flag = sys.call('pgrep -af "sxray/.*'.. app_name ..'" >/dev/null')
	if flag == 0 then
		sys.call("/etc/init.d/sxray stop")
	end
	...
	sys.call(cmd_rm_tmp)
	if flag == 0 then
		sys.call("/etc/init.d/sxray restart >/dev/null 2>&1 &")
	end
...
function get_version()
	local version = sys.exec("opkg list-installed luci-app-sxray 2>/dev/null | awk '{print $3}'")
	if not version or #version == 0 then
		version = sys.exec("apk list luci-app-sxray 2>/dev/null | awk '/installed/ {print $1}' | cut -d'-' -f4-")
	end
	return (version or ""):gsub("\n", ""):match("^([^-]+)")
end

function to_check_self()
	local url = "https://raw.githubusercontent.com/Openwrt-Passwall/openwrt-sxray/main/luci-app-sxray/Makefile"
	local tmp_file = "/tmp/sxray_makefile"
	...
	local remote_version = sys.exec("echo -n $(grep '^PKG_VERSION' /tmp/sxray_makefile | head -n 1 | awk -F '=' '{print $2}')")
	exec("/bin/rm", {"-f", tmp_file})
