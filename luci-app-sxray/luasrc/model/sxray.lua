...
module("luci.model.sxray", package.seeall)
...
local gfwlist_file = "/etc/sxray/gfwlist.txt"
local chnroute_file_ipv4 = "/etc/sxray/chnroute.txt"
local chnroute_file_ipv6 = "/etc/sxray/chnroute6.txt"

function generate_gfwlist()
	local gfwlist_mirror = uci:get("sxray", "main_transparent_proxy", "gfwlist_mirror") or "github"
...
function generate_routelist()
	local apnic_delegated_mirror = uci:get("sxray", "main_transparent_proxy", "apnic_delegated_mirror") or "apnic"
...
function get_routelist_status()
	...
	local chnroute6_time = util.exec("date -r %s '+%%Y/%%m/%%d %%H:%%M:%%S'" % util.shellquote(chnroute_file_ipv6))
