...
m = Map(appname, "%s - %s" % { translate("SXray"), translate("Subscribe") })
...
s = m:section(TypedSection, "subscribe_list", "",
	"<font color='red'>" .. translate("When adding a new subscription, please save and apply before manually subscribing. If you only change the subscription URL, you can subscribe manually, and the system will save it automatically.") .. "</font>")
s.addremove = true
s.anonymous = true
s.sortable = true
s.template = "cbi/tblsection"
s.extedit = api.url("subscribe", "%s")
...
o.write = function(self, section, value)
	...
	if old ~= value then
		m.uci:foreach(appname, "outbound", function(e)
...
o.cfgvalue = function(t, n)
	...
	local num = 0
	m.uci:foreach(appname, "outbound", function(s)
...
m:append(Template(appname .. "/subscribe/js"))
